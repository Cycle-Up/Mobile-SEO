...
import { PRODUCTS, shopNameFor } from '@/lib/pureaqua-products.mjs';

type Product = (typeof PRODUCTS)[number];

/**
 * Productkaart voor PureAqua- en PureFilter-producten: eerlijke omschrijving, prijs alleen
...
export function ProductCard({ product, content }: { product: Product; content: string }) {
  ...
  const shop = shopNameFor(product);
    ...
    <div className="border border-gray-100 rounded-2xl p-5 flex flex-col gap-3 hover:border-[#BAE6FD] hover:shadow-sm transition-all bg-white">
      ...
      {product.price && (
        <p className="text-sm text-gray-800">
          ...
          <span className="text-xs text-gray-400"> (prijs bij publicatie, zie actuele prijs bij {shop})</span>
        </p>
      )}
