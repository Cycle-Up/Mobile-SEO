...
const goodAnchor = '<a href="https://shop.pureaqua.nl/collections/waterontharders?utm_source=waterfilterplatform&amp;utm_medium=affiliate&amp;utm_campaign=waterontharder" rel="sponsored">Bekijk</a>';
...
test('missing rel=sponsored is flagged', () => {
  const html = `<html><a href="https://shop.pureaqua.nl/?utm_source=waterfilterplatform&utm_medium=affiliate">x</a>${disclosure}</html>`;
  const v = analyzeAffiliate(html, allowed);
  assert.ok(v.some(x => /sponsored/.test(x)));
});

test('missing UTM is flagged', () => {
  const html = `<html><a href="https://shop.pureaqua.nl/" rel="sponsored">x</a>${disclosure}</html>`;
  const v = analyzeAffiliate(html, allowed);
  assert.ok(v.some(x => /UTM/.test(x)));
});

test('a non-allowlisted (possibly fabricated) path is flagged', () => {
  const html = `<html><a href="https://shop.pureaqua.nl/collections/verzonnen?utm_source=waterfilterplatform&utm_medium=affiliate" rel="sponsored">x</a>${disclosure}</html>`;
  const v = analyzeAffiliate(html, allowed);
  assert.ok(v.some(x => /allowlist/.test(x)));
});

test('pages without shop links produce no violations and JSON-LD url is ignored', () => {
  const html = '<html><script type="application/ld+json">{"url":"https://shop.pureaqua.nl"}</script></html>';
