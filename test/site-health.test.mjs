...
test('collectHealth runs and returns numeric metrics', () => {
  ...
  for (const k of ['pageFiles', 'staticRoutes', 'dynamicRoutes', 'mdxCount', 'deadLinks', 'sitemapGaps', 'typographyTotal', 'orphans', 'llmsMissing', 'hubItemListOk', 'hubItemListTotal']) {
...
test('renderMarkdown produces a SITE-HEALTH document', () => {
  const md = renderMarkdown({
    ...
    orphans: 0, llmsMissing: 0, hubItemListOk: 13, hubItemListTotal: 13,
  }, new Date('2026-05-29'));
  assert.match(md, /# SITE-HEALTH/);
  assert.match(md, /Dode interne links/);
  assert.match(md, /Orphan-pagina/);
  assert.match(md, /Hubs met ItemList/);
