...
import { buildPersonSchema, buildReviewTeamSchema } from './authors.mjs';

const CONTEXT = 'https://schema.org';
const DEFAULT_IMAGE = 'https://waterfilterplatform.nl/opengraph-image';
const SOCIAL_LINKS = [];

// Benoemde hoofdauteur (E-E-A-T): Person met profielpagina, in plaats van een
// anonieme redactie-Organization. Bron: lib/authors.mjs (1 bron van waarheid).
const DEFAULT_PERSON_AUTHOR = buildPersonSchema();
const EDITORIAL_TEAM = buildReviewTeamSchema();
...
function enrichArticle(a) {
  const out = { ...a };
  if (!out.author) out.author = DEFAULT_PERSON_AUTHOR;
...
export function buildSchema(props) {
  ...
  if (type === 'FAQPage' && faqItems) {
    ...
    schemaObj = {
      ...
      // Partnerrelatie (waarheidsgetrouw): PureAqua ondersteunt het platform via
      // affiliate-vergoedingen; zichtbaar gemeld op /over-ons en bij elke CTA.
      sponsor: {
        '@type': 'Organization',
