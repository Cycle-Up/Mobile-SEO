...
// PureFilter.nl is de webshop van PureAqua voor het losse PureFilter Mineral+ inline-filter
// en de vervangcartridges. Zelfde eigenaar, zelfde disclosure- en UTM-regels.
export const PUREFILTER_STORE = 'https://purefilter.nl';

/** Alle geverifieerde winkel-origins waarnaar het platform commercieel mag linken. */
export const STORES = [STORE, PUREFILTER_STORE];
...
export const DESTINATIONS = {
  ...
  // 4. Plug & play osmose (Countertop). Productpagina verhuisd naar /products/water-zuiver-apparaat
  // (Shopify-handle bevestigd 24-09-2026). anchors: "osmose-apparaat zonder installatie",
  // "countertop osmosesysteem", "water zuiver apparaat"
  countertopRo: {
    key: 'countertopRo',
    label: 'Bekijk het Water zuiver apparaat bij PureAqua',
    url: `${STORE}/products/water-zuiver-apparaat`,
    type: 'product',
    evidenceStatus: 'verified',
  },
  // 5a. Waterontharder PRIMAIR (commerciele focus PureAqua). Let op: Joep gebruikt zout en een stopcontact.
  // anchors: "Joep waterontharder", "compacte waterontharder", "waterontharder voor thuis"
  joep: {
    key: 'joep',
  ...
  // 7. Inline-filter onder het aanrecht, zonder omgekeerde osmose (geen afvalwater, geen tank).
  // Canonieke product-URL door PureAqua aangeleverd (2026-09-24).
  // anchors: "PureFilter Mineral+", "inline waterfilter onder het aanrecht", "filter zonder osmose"
  purefilterMineral: {
    key: 'purefilterMineral',
    label: 'Bekijk PureFilter Mineral+ op PureFilter.nl',
    url: `${PUREFILTER_STORE}/products/purefilter-mineral-waterfilter`,
    type: 'product',
    evidenceStatus: 'verified',
  },
};
...
/** Alle geverifieerde shop-paden (zonder query). */
export function allowedShopPaths() {
  return Object.values(DESTINATIONS).map(d => new URL(d.url).pathname || '/');
}

/** Alle geverifieerde shop-URL's als origin + pad (zonder query) - gebruikt door de affiliate-gate. */
export function allowedShopUrls() {
...
/** Partnernaam voor analytics op basis van de hostnaam, of null voor niet-winkel-URL's. */
export function partnerForHost(hostname) {
  if (hostname === 'pureaqua.nl') return 'pureaqua';
  if (hostname === 'purefilter.nl') return 'purefilter';
  return null;
}
...
export function affiliateClickPayload(href) {
  ...
  const partner = partnerForHost(url.hostname);
