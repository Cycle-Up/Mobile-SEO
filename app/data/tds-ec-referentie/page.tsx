...
const KEY = "tds-ec-referentie";
const ds = DATASETS[KEY];

export const metadata: Metadata = {
  title: "TDS/EC-referentiewaarden per watertype (dataset)",
  ...
  openGraph: { title: "TDS/EC-referentiewaarden per watertype (dataset)", description: "Indicatieve bereiken voor opgeloste stoffen (TDS) per watertype. Werkelijke waarden verschillen per bron.", url: 'https://waterfilterplatform.nl/data/' + KEY, type: 'website', locale: 'nl_NL' },
};

export default function TdsEcReferentieDataset() {
  return (
    <>
          ...
          <table className="w-full min-w-[640px] text-sm border-collapse">
            <thead>
              <tr className="bg-[#E0F2FE]">
                  <th scope="col" className="text-left py-3 px-3 font-semibold text-[#003F5C]">Watertype</th>
                  <th scope="col" className="text-left py-3 px-3 font-semibold text-[#003F5C]">TDS-bereik (mg/L)</th>
                  <th scope="col" className="text-left py-3 px-3 font-semibold text-[#003F5C]">Toelichting</th>
              </tr>
            </thead>
            <tbody>
              {ds.rows.map((row, i) => (
