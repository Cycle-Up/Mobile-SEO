...
- [x] A4. Orphans wegwerken - voeg per wees-pagina >=1 contextuele interne link toe vanuit een relevante hub of "Gerelateerde onderwerpen"-sectie (geen linkdumps; thematisch passend). Klaar wanneer: `node scripts/check-orphans.mjs` rapporteert 0 orphans (of een gemotiveerde whitelist in het script); `npm run build` groen.
