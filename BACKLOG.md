...
- [x] C9. Long-tail ronde B: 4 nieuwe MDX (kraanwater-waterijs-maken, waterfilter-stoomreiniger, kraanwater-kombucha-maken, kraanwater-zuurdesem-starter; kandidaten tandenpoetsen/hardloop-marathon/wasdroger geskipt wegens kannibalisatie/zwak onderwerp) - klaar wanneer: idem.
...
- [x] K5. `lint` als gate aan `npm run verify` toevoegen - klaar wanneer: `npm run verify` draait eslint en is exit 0 in de schone staat; `npm test` groen.
...
- [x] K6. Unit-test voor `SchemaOrg` (server-render -> valide JSON-LD per type: Article/FAQPage/BreadcrumbList/ItemList) - klaar wanneer: test slaagt; `npm test` groen.
